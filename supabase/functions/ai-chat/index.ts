...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Если ключа нет — используем локальные ответы
    if (!deepseekApiKey) {
...
Вот информация о нейрофото:
НейроФото — это художественные AI-портреты с глубокой проработкой деталей. Я создаю семейные портреты, фото для соцсетей, художественные коллажи. Стоимость от 600 ₽.

Вот информация о сайтах:
Я создаю продающие AI-сайты и лендинги под ключ с современным дизайном, анимациями, формами заявок. Стоимость от 29 900 ₽.

Вот информация о ботах:
Макс боты — это умные боты для автоматизации заявок, ответов на вопросы клиентов, сбора лидов. Стоимость от 39 900 ₽.

Вот информация о карточках для маркетплейсов:
Я создаю карточки для Wildberries и Ozon с AI-фонами, инфографикой. Стоимость от 3 500 ₽ за карточку.

Отвечай вежливо, кратко и по делу. Если не знаешь ответа — предложи написать на почту oksanchik2170@yandex.ru.`
          },
          ...messages
        ],
        max_tokens: 500,
    ...
    if (!response.ok) {
      ...
      // Если ошибка — используем локальные ответы
      const userMessages = messages.filter(m => m.role === 'user');
      const userText = userMessages.length > 0 ? userMessages[userMessages.length - 1].content : '';
      const reply = getLocalResponse(userText, messages);
      return new Response(
        JSON.stringify({ reply: `⚠️ ${data.error?.message || 'Ошибка API'}\n\n${reply}` }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
      );
    ...
    const reply = data.choices?.[0]?.message?.content || 'Извините, я не смогла сформулировать ответ.';
...
// --- Локальные ответы (резервный вариант) ---
